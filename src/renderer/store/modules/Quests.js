...
const mutations = {
  VALIDATE_QUESTS (state, currentTimestamp = (new Date()).getTime()) {
...
const actions = {
  PARSE_QUESTS_LIST (context, data) {
    ...
    const {headers} = data
    const JPDate = (new Date(headers.date)).getTime()
    context.dispatch('QUEST_UPDATE_TIMERS', JPDate)
  },
  ...
  QUEST_UPDATE_TIMERS (context, timestamp = (new Date()).getTime()) {
    if (resetTimers) {
    ...
    resetTimers = setTimeout(() => {
      ...
      context.commit('VALIDATE_QUESTS', timestamp + timeout)
      context.dispatch('QUEST_UPDATE_TIMERS')
    }, timeout)
  }
}
...
mutations.VALIDATE_QUESTS(state)
